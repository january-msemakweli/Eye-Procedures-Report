...
# Read the CSV file
data = []
with open('ifakara_csv.csv', 'r', encoding='utf-8') as f:
...
for row in data:
    ...
    if preop_va:
        ...
        # Store patient info for improvement tracking
        if preop_va and week2_va:
...
all_va_values = sorted(
    set(list(preop_va_count.keys()) + list(day1_va_count.keys()) + list(week2_va_count.keys())),
    key=get_va_order
)
...
plt.tight_layout()
plt.savefig('chart_conditions_top10.png', dpi=300, bbox_inches='tight')
plt.close()
...
plt.grid(axis='y', alpha=0.3, linestyle='--')
plt.tight_layout()
plt.savefig('chart_procedures.png', dpi=300, bbox_inches='tight')
plt.close()
...
for va in all_va_values:
    ...
    # Calculate change
    if preop_count > 0 and week2_count > 0:
        change = ((week2_count - preop_count) / preop_count * 100)
        change_str = f"{change:+.2f}%"
    elif preop_count == 0 and week2_count > 0:
        change_str = "+∞%"
    elif preop_count > 0 and week2_count == 0:
        change_str = "-100%"
    else:
    ...
    markdown += f"| {va} | {preop_count} ({preop_pct:.2f}%) | {day1_count} ({day1_pct:.2f}%) | {week2_count} ({week2_pct:.2f}%) | {change_str} |\n"
...
for patient in patient_improvements:
    ...
    week2_score = get_va_score(patient['week2_va'])
    improvement = preop_score - week2_score  # Positive = improvement
    patient['improvement'] = improvement
    patient['achieved_6_6'] = (patient['week2_va'] == '6/6')
...
for patient in top_10_patients:
    markdown += f"| {patient['patient_id']} | {patient['name']} | {patient['preop_va']} | {patient['week2_va']} |\n"
...
plt.grid(axis='y', alpha=0.3, linestyle='--')
plt.tight_layout()
plt.savefig('chart_gender_comparison.png', dpi=300, bbox_inches='tight')
plt.close()
...
plt.grid(axis='y', alpha=0.3, linestyle='--')
plt.tight_layout()
plt.savefig('chart_age_comparison.png', dpi=300, bbox_inches='tight')
plt.close()

markdown += """![Age Group Comparison](chart_age_comparison.png)
"""
